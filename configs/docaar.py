...
@dataclass
class DatasetDocAARConfig(DatasetConfig):
    num_nodes: int
    num_tasks: int
...
@dataclass
class DocAARConfig(Config):
    dataset: DatasetDocAARConfig  # number of classes in video dataset
...
@dataclass
class DocAARPseudoConfig(DocAARConfig):
...
@dataclass
class DocAARDownstreamConfig(DocAARConfig):
